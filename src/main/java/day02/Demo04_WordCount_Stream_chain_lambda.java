...
public class Demo04_WordCount_Stream_chain_lambda {
...
//        reduce.print();
        ...
        SingleOutputStreamOperator<Tuple2<String, Integer>> sum = source.flatMap((String line, Collector<String> collector) -> {
            String[] wordArr = line.split(",");
            for (String s : wordArr) {
                collector.collect(s);
            }
        }).returns(Types.STRING).map((String s) -> {
            return Tuple2.of(s, 1);
        }).returns(Types.TUPLE(Types.STRING,Types.INT)).keyBy((Tuple2<String, Integer> tuple2) -> {
            return tuple2.f0;
        }).sum(1);
        sum.print();
        // todo 4.数据输出Sink
